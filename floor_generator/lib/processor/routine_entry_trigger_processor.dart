...
class RoutineEntryTriggerMethodProcessor implements Processor<RoutineEntryTrigger> {
  ...
  RoutineEntryTrigger process() {
    ...
    if (superType == null || superType.typeArguments.length != 1) {
      throw InvalidGenerationSourceError('Routine class should extends from RoutineEntryTriggerBase',
          todo: 'Please extends from RoutineEntryTriggerBase.', element: _classElement);
    }
    ...
    if (entity == null) {
      throw InvalidGenerationSourceError('Entity is not declared in database entities.',
          todo: 'Declare entity `${entityRoutine!.name}` in database.', element: _classElement);
    }
